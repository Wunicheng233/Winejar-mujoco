...
from mujoco_xarm6.production_demo.line import COVER_CLAMPED_ANGLE_RAD, COVER_TIED_ANGLE_RAD
...
def main() -> None:
    ...
    controller.transition(1, COVER_CLAMPED_ANGLE_RAD, 0.36)
    while controller.transitions:
        controller.advance(model.opt.timestep)
    np.testing.assert_allclose(controller.angles(1), COVER_CLAMPED_ANGLE_RAD, atol=1e-9)

    controller.transition(1, COVER_TIED_ANGLE_RAD, 0.10)
    elapsed = 0.0
    while controller.transitions:
        controller.advance(model.opt.timestep)
    ...
    np.testing.assert_allclose(controller.angles(1), COVER_TIED_ANGLE_RAD, atol=1e-9)
    ...
    for contact in data.contact[: data.ncon]:
        ...
        cover_jar = (
            is_first_jar_cover(first) and second.startswith("wine_jar")
        ) or (
            is_first_jar_cover(second) and first.startswith("wine_jar")
        )
        if (cross_layer or cover_jar) and contact.dist < -0.0005:
